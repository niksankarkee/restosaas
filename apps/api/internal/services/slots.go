...
type Slot struct { Start time.Time; End time.Time; Available int }
...
func GenerateSlots(gdb *gorm.DB, restaurantID string, date time.Time) ([]Slot, error) {
	var r db.Restaurant
	if err := gdb.First(&r, "id = ?", restaurantID).Error; err != nil { return nil, err }
	...
	H1,M1 := parse(oh.OpenTime); H2,M2 := parse(oh.CloseTime)
